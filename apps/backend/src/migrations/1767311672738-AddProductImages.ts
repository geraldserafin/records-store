...
export class AddProductImages1767311672738 implements MigrationInterface {
    name = 'AddProductImages1767311672738'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`order_items\` (\`id\` int NOT NULL AUTO_INCREMENT, \`quantity\` int NOT NULL, \`priceAtPurchase\` float NOT NULL, \`orderId\` int NULL, \`productId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`orders\` (\`id\` int NOT NULL AUTO_INCREMENT, \`guestEmail\` varchar(255) NULL, \`shippingName\` varchar(255) NOT NULL, \`shippingAddress\` varchar(255) NOT NULL, \`shippingCity\` varchar(255) NOT NULL, \`shippingPostalCode\` varchar(255) NOT NULL, \`shippingCountry\` varchar(255) NOT NULL, \`totalAmount\` float NOT NULL DEFAULT '0', \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`userId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
