...
export default function ArtistManager(props) {
  const [editingId, setEditingId] = createSignal(null);
  const [isFormOpen, setIsFormOpen] = createSignal(false);
  const [loading, setLoading] = createSignal(false);
  const [form, setForm] = createSignal({ name: "", slug: "", bio: "", image: "" });
  ...
  return (
    <div class="space-y-6">
      <div class="flex justify-between items-center">
        <h2 class="text-2xl font-semibold text-gray-800">Artists</h2>
        <button
          onClick={openCreate}
      ...
      <Show when={isFormOpen()}>
        <div class="bg-gray-50 border border-gray-200 rounded-lg p-6 animate-in slide-in-from-top-4 duration-200">
          <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-medium">{editingId() ? "Edit Artist" : "New Artist"}</h3>
                  ...
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-3">
                      <Show when={artist.image}>
                        <img src={artist.image} alt={artist.name} class="w-8 h-8 rounded-full object-cover bg-gray-100" />
                      </Show>
                      <span class="font-medium text-gray-900">{artist.name}</span>
                    </div>
                  </td>
                  <td class="px-6 py-4 text-gray-500">{artist.slug}</td>
